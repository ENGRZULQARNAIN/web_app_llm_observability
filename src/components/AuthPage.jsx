...
const AuthPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!isLogin && data.status === 'ok') {
        ...
        setApiMessage({
          text: data.message,
          type: data.status === 'success' || data.status === 'ok' ? 'success' : 'error'
        });
      }
  ...
  const isPasswordMatch = isLogin || formData.password === formData.confirmPassword;
  const isFormValid = isLogin 
    ? formData.email && formData.password
    : formData.name && formData.email && formData.password && formData.confirmPassword && isPasswordMatch;
